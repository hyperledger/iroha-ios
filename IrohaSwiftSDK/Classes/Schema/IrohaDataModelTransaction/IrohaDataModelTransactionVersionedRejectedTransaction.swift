...
public indirect enum VersionedRejectedTransaction: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        switch index {
        case 0:
            let val0 = try container.decode(IrohaDataModelTransaction._VersionedRejectedTransactionV1.self)
            self = .v1(val0)
            break
        default:
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        try container.encode(VersionedRejectedTransaction.index(of: self))
        switch self {
        case let .v1(val0):
